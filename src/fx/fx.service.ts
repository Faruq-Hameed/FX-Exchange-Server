...
export class FxService implements OnModuleInit {
  ...
  private async fetchExternalRates(baseCurrency: string): Promise<Record<string, number>> {
    try {
  ...
  private async saveExchangeRate(baseCurrency: string, targetCurrency: string, rate: number): Promise<void> {
  ...
  async getExchangeRate(fromCurrency: string, toCurrency: string): Promise<number> {
    ...
    throw new BadRequestException(`Could not determine exchange rate from ${fromCurrency} to ${toCurrency}`);
  }
  ...
  async getAllRates(): Promise<Record<string, Record<string, number>>> {
    ...
    for (const baseCurrency of this.supportedCurrencies) {
      ...
      for (const targetCurrency of this.supportedCurrencies) {
        if (baseCurrency !== targetCurrency) {
          allRates[baseCurrency][targetCurrency] = await this.getExchangeRate(baseCurrency, targetCurrency);
        }
      }
    }
